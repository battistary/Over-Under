...
void autonomous() {
    ...
    if ( auton == 1 ) {
        ...
        /* Garunteed awp
        chassis.setPose(49, -58, 45);          // Start
        ...
        chassis.moveTo(60, -47, 750);
        chassis.moveTo(60, -20, 1000);          // Ram real good
        chassis.moveTo(60, -47, 750);           // Back away*/

        // First three triballs 
        chassis.setPose(12, -60, 270);          // Start
        chassis.moveTo(6, -60, 200);            // Move slightly foreward towards first triball
        intake.move(127);                       // Drop intake
        pros::delay(100);                       // Grab 1st triball
        chassis.moveTo(34, -60, 1000, false, 100);           // Move towards match load zone & push preload
        wingBackLeft.set_value(1);              // Extend left wing
        chassis.moveTo(41, -60, 500);           // Align to knock out match load triball
        chassis.moveTo(55, -52, 500);           // Align to knock out match load triball
        ...
        chassis.moveTo(65, -39, 500);           // Move to align better for ram
        chassis.turnTo(65, 0, 400, true);       // Turn for ram
        chassis.moveTo(65, 0, 600);             // Ram first two triballs with vigor
        chassis.moveTo(65, -40, 500);           // Back up from goal
        chassis.turnTo(65, 0, 600);             // Turn to goal
        intake.move(-127);                      // BLAUUGGH -> spit triball
        pros::delay(500);                       // Wait 500 milliseconds
        chassis.moveTo(65, 0, 700);             // Ram last triball with vigor
        pros::delay(10);
        chassis.moveTo(65, -30, 600);           // Back up from goal -27 -> -31
        chassis.setPose(58, -41, imu.get_heading() - 90);   // -37 -> -41

        // Last three triballs
        //chassis.turnTo(34, -30, 400);         // Turn to point towards 4th triball
        ...
        chassis.moveTo(12, -26, 1000);           // Move to 4th triball
        ...
        intake.move(127);                       // Spin intake
        chassis.moveTo(5, -3, 500);             // Intake 5th triball
        ...
        wingBackRight.set_value(1);             // Extend right wing
        pros::delay(300);
        chassis.moveTo(47, -7, 800);            // Ram last three triballs into goal
        chassis.moveTo(20, -7, 500);            // Back up from goal 
    }
        ...
        chassis.setPose(-45, -60, 135);         // Start
        wingBackRight.set_value(1);                 // Extend wing
        pros::delay(500);                       // Wait 500 milliseconds
        chassis.turnTo(72, -60, 500);           // Turn to knock out triball
        pros::delay(500);                       // Wait 500 milliseconds
        wingBackRight.set_value(0);                 // Retract wing
        ...
        chassis.moveTo(-15, -63, 1500, 100);    // Contact elevation bar
        //chassis.moveTo(-40, -58, 1500, 100);  // Drive back for elims
        //chassis.turnTo(-24, 0, 300);
    }
        ...
        chassis.moveTo(-16, -27, 750);          // Move to align for ram
        wingBackRight.set_value(1);             // Extend right wing
        chassis.turnTo(40, -64, 500);           // Turn for ram
        pros::delay(200);
        wingBackLeft.set_value(1);              // Extend left wing
        chassis.moveTo(-72, 0, 1000);      // Ram
        chassis.moveTo(-16, -32, 1000);         // Back up
        
        // 4th ram set
        //chassis.turnTo(-16, -72, 300);        // Turn to move for final ram
        //chassis.moveTo(-16, 10, 1000);        // Move for final ram
        chassis.turnTo(2, -16, 500);            // Turn for next ram
        wingBackLeft.set_value(0);              // Retract left wing
        wingBackRight.set_value(0);             // Retract right wing
        chassis.moveTo(-36, -40, 750);          // Move to match-load bar
        //chassis.turnTo(-27, -63, 500);          // Turn to align with goal
        //chassis.moveTo(-27, -63, 500);          // Move to scoop triballs
        chassis.turnTo(-36, 0, 500);
        chassis.moveTo(-36, -63, 500);
        chassis.turnTo(0, -63, 500);            // Turn to PUSH
        wingBackLeft.set_value(1);
        //chassis.turnTo(43, -60, 500);
        chassis.moveTo(-43, -63, 500);
        chassis.turnTo(-20, -80, 500);
        chassis.moveTo(-64, -43, 750);          // Move along match-load bar
        chassis.turnTo(-64, -72, 500);          // Turn to goal
        wingBackLeft.set_value(1);              // Extend left wing
        intake.move(-127);                      // Spit out any triballs
        chassis.moveTo(-64, 0, 1000);           // Ram
        chassis.moveTo(-64, -43, 500);          // Back up
        chassis.moveTo(-64, 0, 1000);           // Ram
        chassis.moveTo(-64, -43, 500);          // Back up
        wingBackLeft.set_value(0);              // Retract left wing
        intake.move(0);
